...
function getToolLabel(toolName: string, args: any): string {
  const file = args?.path ? args.path.replace(/^\//, "") : null;

  if (toolName === "str_replace_editor") {
    switch (args?.command) {
      case "create": return file ? `Creating ${file}` : "Creating file";
      case "str_replace": return file ? `Updating ${file}` : "Updating file";
  ...
  if (toolName === "file_manager") {
    switch (args?.command) {
      case "rename": return file ? `Renaming ${file}` : "Renaming file";
  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center min-h-[60vh]">
        <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-blue-50 mb-6 shadow-sm mx-auto">
          <Bot className="h-8 w-8 text-blue-600" />
        </div>
        <p className="text-neutral-900 font-semibold text-lg mb-3 text-center">Start a conversation to generate React components</p>
        <p className="text-neutral-500 text-sm max-w-sm text-center mx-auto">I can help you create buttons, forms, cards, and more</p>
      </div>
    );
  }
